...
{
    public class TreatmentMapp
    {
        public static List<TreatmentObj> MappTreatment(DataTable dto)
        {
            ...
            var list = new List<TreatmentObj>();
            ...
            foreach (DataRow row in dto.Rows)
            {
                ...
                int duration = 0;
                if (dto.Columns.Contains("Duracion") && row["Duracion"] != DBNull.Value)
                {
                    try { duration = Convert.ToInt32(row["Duracion"]); } catch { duration = 0; }
                }
                ...
                var item = TreatmentObj.Create(id, name);
                if (item != null)
                {
                    item.SetInformationAditional(duration, description);
                    list.Add(item);
                }
            }
